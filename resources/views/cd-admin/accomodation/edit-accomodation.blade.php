  ...
  <div class="col-xs-12">
   <div class="box">
    <div class="box-header">
      <h3 class="box-title">Edit Accomodation Details</h3>
    </div>
    <form action="{{route('accomodationupdate',$accomodation->id)}}" method="POST" enctype="multipart/form-data">
      @csrf
      <form role="form">
        <div class="box-body">
          <div class="form-group">
            <label for="exampleInputEmail1">Title</label>
            <input type="text" class="form-control" id="exampleInputEmail1" name="title" placeholder="Edit Title" value="{{$accomodation['title']}}">
            <div class="text text-danger">{{ $errors->first('title') }}</div>
          </div>
          <div class="form-group">
            <label for="exampleInputEmail1">Head Description</label>
            <textarea class="summernote" placeholder="Head Description" name="headdescription" >{{$accomodation['headdescription']}}</textarea>
            <div class="text text-danger">{{ $errors->first('headdescription') }}</div>
          </div>
          <div class="form-group">
            <label for="exampleInputEmail1">Body Description</label>
            <textarea class="summernote" placeholder="Body Description" name="bodydescription" >{{$accomodation['bodydescription']}}</textarea>
